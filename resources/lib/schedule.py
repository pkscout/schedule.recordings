...
class Main:
    def __init__(self, thepath):
        ...
        if self.ARGS.action == 'schedule':
            ...
            self.LW.log(['no matching action for %s, exiting' %
    ...
    def _parse_argv(self):
        ...
        parser.add_argument(
            "-a", "--action", help="Action for the script to take")
        parser.add_argument(
            "-t", "--tvmazeids", help="TV Maze IDs (comma sep), 'followed', or 'tags:tagids (comma sep)")
        parser.add_argument("-r", "--recordingid",
                            help="The unique recording id provided by the PVR")
        parser.add_argument(
            "-l", "--lookforward", help="number of days forwards in time to look for episode match")
    ...
    def _check_recurring(self, show):
        ...
        if not recurrings:
            self.LW.log(
                ['no recurring recordings found, trying to schedule recording'], 'info')
            return False
        self.LW.log(
            ['found some recurring recordings, checking to see if one matches %s' % show['name']], 'info')
        if show['name'] in recurrings:
            self.LW.log(
                ['found a matching recurring recording, skipping'], 'info')
            return True
        self.LW.log(
            ['no matching recurring recording, trying to schedule recording'], 'info')
        return False
    ...
    def _check_upcoming_episode(self, show):
        ...
        if not success:
            self.LW.log(
                ['unable to get episode information from TVMaze for episode %s, skipping' % episodeid], 'info')
            return False
        ...
        if next_airdate:
            ...
            if gapdays > self.LOOKFORWARD:
                self.LW.log(
                    ['next episode is still %s days away, skipping for now' % str(gapdays)], 'info')
                return False
            else:
                self.LW.log(
                    ['found an episode within %s days, trying to schedule' % str(gapdays)], 'info')
                return True
        else:
            self.LW.log(
                'no airdate for next episode in TVMaze, skipping', 'info')
            return False

    def _mark_aquired(self):
        self.LW.log(['starting process to mark show as acquired',
                    'getting show information from DVR'], 'info')
        show_info, loglines = self.DVR.getShowInformationFromRecording(
            self.ARGS.recordingid)
        if show_info:
            self.LW.log(
                ['show info found', 'getting followed shows from TV Maze'], 'info')
            success, loglines, results = self.TVMAZE.getFollowedShows(
                params={'embed': 'show'})
            self.LW.log(loglines)
            if not success:
                self.LW.log(
                    ['no valid response returned from TV Maze, aborting'], 'info')
                return
            tvmazeid = ''
            show_override = config.Get('show_override')
            self.LW.log(
                ['checking to see if there is an override for %s' % show_info['name']], 'info')
            try:
            ...
            for followed_show in results:
                try:
                    ...
                    continue
                self.LW.log(['checking for %s matching %s' %
                            (show_info['name'], followed_name)], 'info')
                if followed_name == show_info['name']:
                    self.LW.log(['found match for %s' %
                                show_info['name']], 'info')
                    tvmazeid = followed_show['show_id']
                    break
            if tvmazeid:
                self.LW.log(['found tvmazeid of %s' %
                            tvmazeid, 'attempting to get episode id'], 'info')
                params = {'season': show_info['season'],
                          'number': show_info['episode']}
                success, loglines, results = self.TVMAZE.getEpisodeBySeasonEpNumber(
                    tvmazeid, params)
                self.LW.log(loglines)
                if not success:
                    self.LW.log(
                ...
                try:
                    ...
                    episodeid = ''
                if episodeid:
                    self.LW.log(['got back episode id of %s' % episodeid,
                                'marking episode as acquired on TV Maze'], 'info')
                    success, loglines, results = self.TVMAZE.markEpisode(
                        episodeid, marked_as=1)
                    self.LW.log(loglines)
                    if not success:
                        self.LW.log(
                            ['no valid response returned from TV Maze, show was not marked'], 'info')
                else:
    ...
    def _schedule_recordings(self):
        ...
        for item in items:
            if use_tvmaze_public:
                ...
                if not success:
                    self.LW.log(
                        ['got nothing back from TVMaze, aborting'], 'info')
                    break
            ...
            if self._check_upcoming_episode(show):
                if not self._check_recurring(show):
                    success, loglines = self.DVR.scheduleNewRecurringRecording(
                        show['name'], config.Get('dvr_params'))
                    if success and tag_map and config.Get('tvmaze_untag'):
                        self.LW.log(['untagging show %s with tag %s' %
                                    (item, tag_map[item])], 'info')
                        self.TVMAZE.unTagShow(item, tag_map[item])
                        self.LW.log(loglines)
                        if not success:
                            self.LW.log(
                                ['no valid response returned from TV Maze, show was not untagged'], 'info')

    def _get_followed(self):
        self.LW.log(
            ['trying to get a list of followed shows from TV Maze'], 'info')
        items = []
        success, loglines, results = self.TVMAZE.getFollowedShows(
            params={'embed': 'show'})
        self.LW.log(loglines)
    ...
    def _get_tagged(self):
        self.LW.log(
            ['tring to get a list of tagged shows from TV Maze'], 'info')
        items = []
        tag_map = {}
        try:
        ...
        for tag in tags:
            ...
            if not success:
                self.LW.log(
                    ['no valid response returned from TV Maze, skipping %s' % tag], 'info')
                continue
            if self._check_results(results):
                for show in results:
                    try:
